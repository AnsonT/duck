...
//Define if there is a quackPack for this device // lala Magus test
#define QUACKPACKSIM
...
void setupLoRa()
{
  ...
  else
  {
   ...
   LoRa.setSyncWord(0xF5);         // ranges from 0-0xFF, default 0x34
...
{
  String payloadLength = deviceID + messageID + payload + qtest.path;
  if(payloadLength.length() > 250)
  {
    Serial.println("The length of the payload is too long. Defaulting path to device ID");
    couple(user_ID, deviceID);
    couple(message_ID, messageID);
    couple(quacket_B, payload);
    couple(path_Q, deviceID);
  }
  else
  {
    ...
    if(qtest.path != NULL)
    {
      Serial.println("Printing path: " + qtest.path);
      couple(path_Q, qtest.path);
    }
    else
    {
      Serial.println("1st device added to path: " + deviceID);
      couple(path_Q, deviceID);
    }
    Serial.println("Payload: " + deviceID + messageID + payload + qtest.path);
    LoRa.endPacket();
  }
}
...
void receive(int packetSize)
{
  if (packetSize != 0)
  {
    ...
    while (LoRa.available())
    {
      ...
      else if (byteCode == path_Q)
      {
        qtest.path = readMessages(mLength);
      }
      else if (byteCode == iamhere)
      { //DetectorDuck
